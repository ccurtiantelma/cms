 ...
 * decisione formale: l'integrazione con le Collezioni/Dynamic Tags, l'engine
 * di visibilità condizionale (`displayConditions`/`shouldRenderBlock`,
 * nessuna ADR nel dominio blocchi — `ADR-88` copre un dominio diverso, le
 * condizioni di assegnazione dei site template) e qualunque UI di selezione
 * token nell'Inspector. Controparte 1:1 di
 * `app/backend/src/common/dynamic-data-token.resolver.ts` (stessa logica,
 * duplicata per assenza di un package condiviso tra le due app — stessa
 * convenzione già in uso per `theme-css.utils.ts`).
 */
