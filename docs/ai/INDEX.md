...
| Auth, Ruoli & Permessi | `docs/ai/adr/ADR-99-rbac-dinamico-ruoli-e-permessi-granulari.md` · `docs/ai/specs/SPEC-RBAC-F1-schema-seed-cache-guard.md` · `docs/ai/plans/PLAN-RBAC-F1-schema-seed-cache-guard.md` · `docs/ai/specs/SPEC-RBAC-F2a-backend-api.md` · `docs/ai/plans/PLAN-RBAC-F2a-backend-api.md` · `docs/ai/specs/SPEC-RBAC-F2b-frontend-ui.md` · `docs/ai/plans/PLAN-RBAC-F2b-frontend-ui.md` · `docs/ai/specs/SPEC-RBAC-F2c-media-permissions.md` · `docs/ai/plans/PLAN-RBAC-F2c-media-permissions.md` · `docs/ai/adr/ADR-18-ownership-per-riga.md` · `docs/ai/adr/ADR-13-gestione-sessioni-dispositivi.md` · `docs/ai/adr/ADR-90-custom-fonts-code-e-role-manager.md` (§ 3, in discussione) · `app/backend/src/permissions/permissions.registry.ts` · `docs/business-rules.md` (A4 e § Permessi editoriali) |
...
- **Fasi RBAC: la "F2b" dei documenti è la F3 di ADR-99 § 11 (frontend)**. `SPEC-RBAC-F2b-frontend-ui.md`
  e `PLAN-RBAC-F2b-frontend-ui.md` (approvati 2026-09-24, marketing@antelmagroup.net) coprono
  permessi in `useAuthStore`, `useHasPermission`/`<Can>`/`RequirePermission`, pagina `/roles` e
  ruoli aggiuntivi in `PageUsers`. L'enforcement `media:*` su `app/files`, chiamato "F2b" in
  `SPEC-RBAC-F2a-backend-api.md` S11 e nella divergenza 5 del PLAN F2a, è **rinominato F2c**
  (SPEC F2b S24) e richiede una SPEC propria, non ancora scritta. La SPEC F2a resta storica e non
  riscritta: la ridenominazione vive in SPEC F2b S24 e in questa nota.
- **F2c (enforcement `media:*`)**: `SPEC-RBAC-F2c-media-permissions.md` e
  `PLAN-RBAC-F2c-media-permissions.md` (approvati il 2026-09-24, marketing@antelmagroup.net).
  `media:upload` protegge `POST app/files`; `media:delete_any` sostituisce la soglia Admin come
  accesso elevato di `softDelete` (ADR-18 invariata per l'autore). Nessun `media:read` (SPEC S39).
